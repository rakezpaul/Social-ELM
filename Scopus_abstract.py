...
for _, row in df.iterrows():
    keywords = row['authorkeywords'].split(';')  # Adjust based on your delimiter
...
# Load your data, create the co-occurrence matrix, and construct the graph `G`
# (as demonstrated in previous messages)

# Example: Assuming G is already defined
...
################################################
# Sort and display the top keywords by degree centrality
sorted_degree_centrality = sorted(degree_centrality.items(), key=lambda x: x[1], reverse=True)
top_degree_keywords = sorted_degree_centrality[:10]  # Top 10 keywords
print("Top 10 Keywords by Degree Centrality:")
print(top_degree_keywords)

# Sort and display the top keywords by closeness centrality
sorted_closeness_centrality = sorted(closeness_centrality.items(), key=lambda x: x[1], reverse=True)
top_closeness_keywords = sorted_closeness_centrality[:10]  # Top 10 keywords
print("Top 10 Keywords by Closeness Centrality:")
print(top_closeness_keywords)

# Sort and display the top keywords by betweenness centrality
sorted_betweenness_centrality = sorted(betweenness_centrality.items(), key=lambda x: x[1], reverse=True)
top_betweenness_keywords = sorted_betweenness_centrality[:10]  # Top 10 keywords
print("Top 10 Keywords by Betweenness Centrality:")
print(top_betweenness_keywords)
...
# Assuming G is your graph created from co-occurrence data
...
node_sizes = [v * 1000 for v in degree_centrality.values()]  # Scale size to make it more visible
...
# Create the plot
plt.figure(figsize=(24, 18), dpi=300)  # Set figure size and dpi
